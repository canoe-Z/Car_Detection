...
from cv2 import cv2
from dets.py_cpu_nms import py_cpu_nms
from dets.common import pyramid
from dets.common import sliding_window
...
def main():
    ...
    for _ in range(100):
        ...
        if frame is None:
            break
        frames.append(frame)
    # imgs = ['data/image/1.jpg', 'data/image/2.jpg', 'data/image/3.jpg',
    #         'data/image/4.jpg', 'data/image/5.jpg', 'data/image/6.jpg']
    ...
    for path in tqdm(frames):
        ...
        for resized in pyramid(img, scale=1.2):  # 对于迭代器迭代出的大小不一的金字塔图片
