...
def draw_bouding_box_yolo(img, bx, by, bw, bh, conf=None):
    xmin, ymin, xmax, ymax = convert_bouding_box(img, bx, by, bw, bh)
    draw_bouding_box(img, xmin, ymin, xmax, ymax, conf)


def draw_bouding_box(img, xmin, ymin, xmax, ymax, conf=None):
    x1 = int(xmin)
    y1 = int(ymin)
    x2 = int(xmax)
    y2 = int(ymax)
    ...
    if conf == None:
        plot_one_box((x1, y1, x2, y2), img, (0, 0, 255), 'car', 3)
    else:
        plot_one_box((x1, y1, x2, y2), img, (0, 0, 255),
                     'car {:.2f}'.format(conf), 3)
